...
public abstract class ScriptControl {
    ...
    public void destroy(Moveable mv){
        destroy(mv.getGameObject()); }
    /**
    ...
    public GameObject instantiate(String path, Vector2 pos, double angle){
        GameObject inst = getNewGameObject(path);
        if (inst == null) return null;
        inst.transform.position = pos;
        inst.transform.angle = angle;
    ...
    public GameObject instantiate(String path, double angle){
        GameObject inst = getNewGameObject(path);
        if (inst == null) return null;
        inst.transform.angle = angle;
        return inst;
